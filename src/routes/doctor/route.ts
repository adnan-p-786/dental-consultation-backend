...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const finalName = (doctorName || req.body.doctor_name || req.body.name || "").trim();
      const finalEmail = (doctorEmail || req.body.doctor_email || req.body.email || "").trim().toLowerCase();
      const finalPhone = (phoneNumber || req.body.phone_number || req.body.phone || "").trim();
      const finalSpecialization = (specialization || "").trim();
      const finalWorkingHours = (workingHours || req.body.working_hours || "").trim();
      ...
      if (req.file) {
        ...
        finalPhoto = (doctorPhoto || req.body.doctor_photo || req.body.avatar).trim();
      } else {
        finalPhoto = "";
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (doctorName || req.body.name || req.body.doctor_name) {
        updateData.doctorName = (doctorName || req.body.name || req.body.doctor_name).trim();
      }
      if (doctorEmail || req.body.email || req.body.doctor_email) {
        updateData.doctorEmail = (doctorEmail || req.body.email || req.body.doctor_email).trim().toLowerCase();
      }
      if (phoneNumber || req.body.phone || req.body.phone_number) {
        updateData.phoneNumber = (phoneNumber || req.body.phone || req.body.phone_number).trim();
      }
      if (specialization) {
